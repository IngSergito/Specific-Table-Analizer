    ...
    If ventana.Show Then
        ...
        For i = Fila To fila_ultima
            ...
            If alarma_existente = False Then
                ...
                fila_nueva.Range.Columns(3).NumberFormat = "[h]:mm:ss"
                If Not (ThisWorkbook.Sheets("Correlación alarma-observación").ListObjects("Correlación").Range.Find(rango_alarmas.Cells(i, nameCol).Value, LookAt:=xlWhole) Is Nothing) Then
                    fila_nueva.Range.Columns(4).Value = ThisWorkbook.Sheets("Correlación alarma-observación").ListObjects("Correlación").Range.Find(rango_alarmas.Cells(i, nameCol).Value, LookAt:=xlWhole).Offset(0, 1).Value
                End If
                Set fila_nueva = hoja_tablas.ListObjects("AlarmEquip" & rango_alarmas.Cells(i, severidadCol).Value).ListRows.Add(j + 1, True)
                fila_nueva.Range.Font.Bold = False
                fila_nueva.Range.Columns(1).Value = "   " & rango_alarmas.Cells(i, equipCol).Value
...
Sub GenerarWord()
    ...
    ' Crea un objeto que representa la aplicación de Word
    ...
    With Documento
        ...
        .BuiltinDocumentProperties("Title").Value = "Diagnóstico Nivel 2"
        .BuiltinDocumentProperties("Author").Value = "Equipos de Acceso Inalámbrico"
    ...
    ' Insertar el título, asunto y autor en la primera página
    With objWord.Selection
        ...
        .Style = Documento.Styles("Título 1")
        .TypeText ("Capítulo 1 NE: BSC, RNC, BTS, NodosB y eNodosB")
        .TypeParagraph
        .Style = Documento.Styles("Título 2")
        .TypeText ("1.1 Desempeño (análisis de alarmas)")
        .TypeParagraph
        .Font.Bold = False
        .Font.Size = 11
        .Font.Underline = wdUnderlineNone
        .TypeText ("Resumen mensual del análisis del desempeño semanal y acciones acometidas.")
        .TypeParagraph
        .Style = Documento.Styles("Título 3")
    ...
    With objWord.Selection
        .InsertBreak Type:=wdPageBreak
        .Style = Documento.Styles("Título 2")
        .TypeText ("1.2 Tickets de Fallas ocurridas en el período")
        .TypeParagraph
        .TypeParagraph
        .Style = Documento.Styles("Título 3")
        ...
        .TypeText ("entre ellos la mayor causa se encuentra banco de baterías defectuoso, tec cooler, licencia, deterioro de la calidad del servicio y de fan.")
        .TypeParagraph
        .TypeParagraph
